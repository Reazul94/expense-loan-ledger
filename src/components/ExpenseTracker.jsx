...
export default function ExpenseTracker({ 
  ...
  const getCategoryTag = (catId) => {
    ...
    if (!cat) return null;
    return (
      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${cat.color}`}>
        {t[cat.tKey]}
      </span>
    );
  };
            ...
            <div key={dateStr} className="space-y-2.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {groupedExpenses[dateStr].map(item => (
                  <div 
                    key={item.id} 
                    className="glass-card-hover rounded-xl p-4 flex flex-col justify-between border border-slate-800/80 text-xs gap-3 group relative"
                  >
                    <div className="flex justify-between items-start gap-2">
                      <div className="relative group/tooltip flex-1 min-w-0">
                        <span className="font-semibold text-slate-100 line-clamp-2 cursor-help hover:text-indigo-400 transition-colors block">
                          {item.description}
                        </span>
                        {/* Hover Popup Note Tooltip */}
                        <div className="absolute left-0 bottom-full mb-2 hidden group-hover/tooltip:block w-56 p-2.5 rounded-xl text-[10px] backdrop-blur-md shadow-2xl z-50 pointer-events-none break-words whitespace-normal leading-relaxed panel-container">
                          {item.description}
                        </div>
                      </div>
                      <span className="font-bold text-slate-100 shrink-0 text-sm">
                        {formatCurrency(item.amount)}
                      </span>
                    </div>

                    <div className="flex justify-between items-center mt-1 border-t border-slate-800/50 pt-2.5">
                      ...
                      <div className="flex gap-1.5">
                        <button
                          ...
                          className="p-1 rounded text-slate-400 hover:text-indigo-400 hover:bg-indigo-500/10 transition-all duration-200"
      ...
      {selectedNote && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="rounded-2xl p-6 max-w-md w-full shadow-2xl relative space-y-4 max-h-[80vh] flex flex-col panel-container">
            ...
            <div className="overflow-y-auto pr-1 flex-1 text-slate-300 text-xs leading-relaxed max-h-[50vh] break-words whitespace-pre-wrap">
              {selectedNote}
            </div>
